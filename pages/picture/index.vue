  ...
  <div class="picture">
    ...
    <div class="picture_list">
      <div
        ...
        :key="pic.id"
        @click="beforeTodetail(pic)"
      >
        <img
          class="cover"
          :src="
            pic.password != ''
              ? require('../../static/img/clock.jpg')
              : pic.cover == ''
              ? require('../../static/img/timg.jpg')
              : pic.cover
          "
          :title="pic.createAt | dateFormat"
        />
        <div class="pic_title">
          {{ pic.title }}
          <span v-if="pic.password != ''" :style="{ color: '#ccc' }"
            >(已加密)</span
          >
        </div>
        ...
        <div
          class="tag"
          :style="{ borderColor: pic.tagColor }"
          v-if="pic.tag != ''"
        >
          <span class="tagtxt">{{ pic.tag }}</span>
        </div>
      </div>
    </div>
    <el-dialog
...
export default {
  ...
  methods: {
    ...
    beforeTodetail(pic) {
      this.clickPic = pic;
      if (pic.password == "") this.toDetailPic(pic.id);
      else {
    ...
    checkPassword() {
      if (this.password) {
        ...
        request({
          ...
          this.dialogVisible = false;
          this.toDetailPic(this.clickPic.id);
        });
      }
    },
  },
  async asyncData(context) {
    let res = await request({
      url: `/photo/list?page=1&size=99999999`,
    });
    let pictureList = [];
    if (res && res.code == 200) {
      pictureList = res.data.list;
    }
    return {
      pictureList,
    };
