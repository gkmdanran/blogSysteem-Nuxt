<template>
  <div class="chat">
    ...
    <div class="tips">
      如果您发现我的文章内容有误，或者此网站有bug，欢迎在留言区留下您宝贵的建议，方便的话留下您的联系方式哦~
    </div>
    ...
    <div class="commnet_list">
      ...
      >
        <div class="commnet_info">
          ...
          <span class="time">{{ chat.createAt | dateFormat }}</span>
...
export default {
  ...
  methods: {
    ...
    async sendComment() {
      if (!this.form.chatContent.trim()) return;
      if (this.form.chatName == "")
        return this.$refs.toast.shows("请输入您的昵称~");
      if (this.form.chatNumber && !this.form.chatWay)
      ...
      request({
        ...
        if (res.code == 200) {
          ...
          this.getChatList();
          this.form = {
            chatName: localStorage.getItem("danran_nickname"),
            chatWay: "",
            chatNumber: "",
            chatContent: "",
          };
    ...
    getChatList() {
      request({
        url: `/chat/list?page=${this.pageNum}&size=10`,
      }).then((res) => {
        if (res.code == 200) {
